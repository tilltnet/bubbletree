    ...
    function lp(hierarchyRoot, padding, width, height, n, resizing_factor) {

        let min_size = Math.min(width, height);

        var sizeScale = d3.scaleSqrt()
            .range([2, min_size / (Math.sqrt(n) * resizing_factor)]);

        // Circle pack by d3.
        let pack = d3.pack()
            .radius(function(d) { return sizeScale(d.r); })
            .size([width, height]);
     ...
     * Implanckementation.
     */
    function bubbletreemap(width, height, n, resizing_factor, max_textsize) {
        let bubbletreemap,
            padding = 10,
            curvature = 10,
            colormap = [],
        ...
        return bubbletreemap = {
            ...
            doLayout: function() {
                lp(hierarchyRoot, padding, width, height, n, resizing_factor);
                return bubbletreemap;
            },
...
function doIt(width, height, x, r) {
    if(!working) {
        working = true;
        console.log(x);
        ...
        let padding = Number(settings.padding);
        let curvature = Number(settings.smoothness);
        drawChart(jsonData, svg, padding, curvature, width, height, settings.resizing_factor, settings.max_textsize, settings.min_textsize_scale);
        //console.log(jsonData);
        working = false;
    }
}

function resize(width, height, r) {
    if(!working) {
        working = true;
...
function drawChart(data, svg, padding, curvature, width, height, resizing_factor, max_textsize, min_textsize_scale) {
    // Create hierarchy.
    let root = d3.hierarchy(data)
        .sum(function(d) { return d.size; })
        .sort(function(a, b) { return b.value - a.value; });

     let n = d3.hierarchy(data).descendants().filter(
         function (candidate) {return !candidate.children;}).length;
    //console.log(root.sum);

    // Create bubbletreemap.
    let bubbletreemap_ = bubbletreemap(width, height, n, resizing_factor, max_textsize)
        .padding(padding)
        .curvature(curvature)
        .hierarchyRoot(root)
    ...
    let hierarchyRoot = bubbletreemap_.doLayout().hierarchyRoot();
    ...
    contourGroup.selectAll("path")
        .data(bubbletreemap_.getContour())
        .enter().append("path")
            ...
            if("labels" in leafNodes[i].data) {
              ...
              for(let j = leafNodes[i].data.labels.length - 1; j >= 0; j--) {
                  label_array.push({
                    ...
                    "parent_size": leafNodes[i].r,
                    "biggest_in_group": leafNodes[i].data.labels[0].size,
                    "smallest_in_group": leafNodes[i].data.labels[4].size,
